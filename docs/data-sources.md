...
## Traffic-fine lookup sources — Phase 15

Reviewed 2026-09-16. The review found no documented official public API or explicit permission for unattended,
plate-only traffic-fine lookup. The relevant official forms use a user-entered security code or additional case and
vehicle-document fields. Absence of a published robots or automation rule is not permission to automate. TraNhanh
therefore does not scrape these pages, discover protected endpoints, solve CAPTCHA, or submit user plates to a source.

| Name                                                                 | Authority                                                | URL                                                                                                            | Official status                               | Geographic coverage                                                                     | Supported vehicles                                          | Required inputs                                                                                  | CAPTCHA | Authentication                               | Automated-access status | Update semantics                                                                                 | Known limitations                                                                                                                              | Last reviewed |
| -------------------------------------------------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ------- | -------------------------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| Tra cứu phương tiện vi phạm giao thông qua hình ảnh                  | Cục Cảnh sát giao thông, Bộ Công an                      | [CSGT lookup](https://www.csgt.vn/tra-cuu-phuong-tien-vi-pham.html)                                            | Official national traffic-police site         | Vietnam; effective participating-system coverage is not guaranteed by a public contract | Cars, motorcycles, electric bicycles as offered by the form | Full plate, vehicle type, security code                                                          | Yes     | No account shown                             | **MANUAL_ONLY**         | The page does not publish a machine-readable freshness or completeness SLA                       | No documented public API; CAPTCHA requires the person to complete the lookup; a missing record cannot prove there are no violations            | 2026-09-16    |
| Nộp phạt xử lý vi phạm hành chính trong lĩnh vực giao thông đường bộ | Văn phòng Chính phủ / Cổng Dịch vụ công Quốc gia         | [National Public Service Portal](https://vpcp.dichvucong.gov.vn/p/home/dvc-thanh-toan-vi-pham-giao-thong.html) | Official government service                   | Participating administrative-violation payment cases nationwide                         | Cases supported by the payment service                      | Decision number or report details, verification code, and workflow-specific personal information | Yes     | Workflow may require identity or case access | **MANUAL_ONLY**         | Case/payment state follows the service workflow; no public lookup SLA was found                  | Payment workflow rather than a plate-only public API; additional identifiers and personal data make it unsuitable as the default provider      | 2026-09-16    |
| Tra cứu thông tin phương tiện                                        | Cục Đăng kiểm Việt Nam                                   | [Vietnam Register lookup](https://app.vr.org.vn/ptpublic/thongtinptpublic.aspx)                                | Official vehicle-registration/inspection site | Registered vehicles in the Vietnam Register system                                      | Vehicles covered by the registration system                 | Plate, current inspection certificate/sticker number, security code                              | Yes     | No account shown                             | **MANUAL_ONLY**         | Inspection and warning information reflects the register; no public machine-update SLA was found | Requires a private document value; traffic warnings are not established as a complete nationwide traffic-fine result; no documented public API | 2026-09-16    |
| Tra cứu vi phạm                                                      | Phòng Cảnh sát giao thông, Công an Thành phố Hồ Chí Minh | [HCMC traffic-police lookup](https://csgt-congan.hochiminhcity.gov.vn/wps/portal/Home/tra-cuu-vi-pham)         | Official local police site                    | Hồ Chí Minh City source/system coverage                                                 | Vehicle categories offered by the local form                | Plate and verification code; optional email fields appear in related notification flow           | Yes     | No account shown for basic form              | **MANUAL_ONLY**         | Local publication timing and completeness are not covered by a public API contract               | Local coverage, verification step, no documented public API, and no verified permission for unattended access                                  | 2026-09-16    |
| Commercial traffic-fine aggregators                                  | Various non-government publishers                        | No provider selected                                                                                           | Secondary/unverified                          | Varies                                                                                  | Varies                                                      | Often a full plate and vehicle type                                                              | Varies  | Varies                                       | **UNSUITABLE**          | Provenance and refresh schedules vary                                                            | No candidate established authoritative provenance, reusable terms, stable contracts, and better privacy than the official manual flow          | 2026-09-16    |

The Phase 15 default adapter is the official CSGT manual provider. It reports its capabilities and returns
MANUAL_VERIFICATION_REQUIRED with the official URL and limitations; it never makes an upstream request. The endpoint
