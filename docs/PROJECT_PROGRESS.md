...
- The full database suite requires the designated `tranhanh_test` name and was run before restoring reviewed public fixtures; runtime was then run after the four fixture imports. Temporary 390px light and 1440px dark screenshots were generated, dimension-checked, reviewed, and removed. Phase 22 implementation follows below.
- Phase 22 consumes active maintenance history and FuelLogEntry as authoritative cost inputs without introducing duplicate rows.

### Phase 22 — Vehicle Expenses

Status: Complete. Commit: `feat: add vehicle expenses`.

- Added additive `VehicleExpense` schema/migration with manual-only INSURANCE, REGISTRATION, TOLL, PARKING and OTHER
  categories, UUID identity, composite User/Vehicle ownership, account/vehicle cascades, DATE, bounded text, exact
  non-negative BIGINT VND range `0..9999999999999999`, archive audit checks and deterministic indexes. Existing migrations
  were not edited.
- Added shared contracts and private `/api/v1/vehicles/:vehicleId/expenses` summary/ledger plus manual list/create/detail/
  update/archive/restore. Fuel and active maintenance values are read dynamically and never copied; plans, documents,
  reminders, public prices/fines and other records do not create costs. Summary uses Vietnam instant bounds for fuel and
  direct DATE bounds for DATE sources, preserving null unknown maintenance costs separately from zero. Ledger is a bounded
  SQL union with deterministic ties and page size capped at 100.
- Added trusted-origin/access-auth/CSRF guarded owner-scoped API behavior with safe 404s and transactional archived-vehicle
  mutation blocking. Added neutral browser-only bilingual `/vi/garage/:id/chi-phi` and `/en/garage/:id/expenses` routes,
  manual forms/detail/lifecycle controls, source links, Garage summary integration, month/category/source breakdowns,
  loading/empty/error/retry states, exact VND display, and responsive light/dark UI. No private TransferState, storage,
  SEO, sitemap, receipt/OCR, refunds, currency conversion, recurring, budget, dashboard or notification behavior.
- Focused matrix covers exact money/lifecycle/ownership/API/route behavior. The prior Node 24.15.0 gate passed API unit
  238/238, API E2E 51/51, Angular 74/74, format/lint/unit/E2E/Prisma/build checks, API/web production builds, SSR i18n
  and SEO/sitemap smoke checks.
- Added PostgreSQL fixtures/service coverage for Vietnam year/month and instant-vs-DATE boundaries, mixed-source
  summary/tie pagination, null versus zero maintenance, source edits/archive/restore and no mirrored duplicate rows.
- PostgreSQL migration apply/status and the focused Phase 22 service fixture suite pass (9/9). Guarded public lookup
  fixtures now query occupied finite keys and choose deterministic unused candidates, so reviewed public rows are never
  modified and the full runner no longer depends on random collision retries. The full guarded database suite passes
  102/102 twice against local `tranhanh_test`; its public fixture counts remain Phone 57, Area 122, Vehicle Plate 81,
  Postal 3,320, with zero leaked VehicleExpense rows. The Redis/BullMQ runtime gate passes 10/10.
- The Angular regression covers date-only rendering under a negative UTC offset. Installed Chrome 153 via a temporary CDP
  session passed the authenticated synthetic ledger and add-expense form routes, exact VND rendering, private noindex
  metadata, and no horizontal overflow at 390px and 1440px. Broader error/retry/filter/reset/source-link/archive-confirmation
  and screenshot review remain pending until the Playwright package is restored.
